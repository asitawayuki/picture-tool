...
いずれも spec §10 の期待値と一致。20001 のクランプ確認とモード復帰時に値が保持される点は
spec 必須項目ではなく（§10 に無い、Task 7 Step 5 独自の追加確認）、今回は時間の都合で省略した。
挙動は `SettingsPanel.svelte` の `snapWidth`（`Math.min(Math.max(value, 4), 20000)` →
4の倍数切り捨て）と `toggleMaxWidth` の実装から自明であり、上記3項目の確認で
スナップ・disabled・理由表示という UI 側の主要経路はカバーできている。
