...
### Task 14 — 仮想化の余白はスクロールする箱に付けられない（計画の構造を変えた）

**計画（と Task 13）の構造は、そもそもスクロールしていなかった。**
`role="listbox"` の要素に `overflow-y: auto` と
`padding-top: {paddingTop}px` / `padding-bottom: {paddingBottom}px` を
同時に持たせる形だが、**padding はその要素自身の padding box を膨らませる。**
3,000 枚では padding-bottom が 34 万 px になり、要素自体が 340,977px の高さになる
（`clientHeight` も同じ値になる）。スクロールできる量は
`scrollHeight - clientHeight` = 約 2,700px しか無く、7 行目より先へ行けない。

さらに `bind:clientHeight` を**その同じ要素**に張っていたため、
「余白が伸びる → 可視高を測り直す → 描画範囲が広がる → 余白が縮む → …」の
帰還路ができ、可視範囲が `[0,14]` と `[0,2993]` を毎フレーム往復した。
**1 往復ごとに 3,000 件近い取得要求を捨てては積み直していた**
（`e2e/grid.spec.ts` の実行時間が 35 秒 → 6.7 秒になったのはこれが消えた分）。

Task 13 の検査が全部通っていたのは、この振動が「全件描画」の側の位相で
`aria-posinset=3000` まで DOM に出ていたためで、**振動が検査を通していた**。

構造を分けた:

```
.scroller  … overflow-y: auto ／ 可視高はここで測る（余白を持たない）
  .grid    … role="listbox" ／ 仮想化の余白と grid-template-columns はこちら
    [role=option] × 可視分
```

spec §4-1 の「スペーサー要素を listbox の直接の子にしてはならない」はこれで満たす
（listbox の子は option だけ）。**listbox はスクロールする箱ではなくなった**ので、
e2e で位置を動かすときは `.scroller` を掴む（`e2e/grid.spec.ts` の `scrollerOf`）。
Task 15 の `measureScroll` に渡すセレクタも `[role="listbox"]` ではなく
`.scroller` にすること。

一般化すると **「測る対象と変える対象を同じ要素にしない」**。
`bind:clientHeight` を張った要素の寸法を自分で書き換えると必ず帰還路になる。

### Task 14 — フィルムストリップの要素数

**仮想化は入れていない。** `{#each images}` で写真の枚数だけ `<button>` を作る
（3,000 枚なら 3,000 個）。判断の材料:

- 取得は `STRIP_WINDOW = 20`（前後 20 枚）で絞ってあるので IPC は増えない。
  出るのは DOM の要素数と初回レイアウトのコストだけ
- 1 要素あたり 77×96px の空箱で、`.strip` は `overflow-x: auto` の flex 行。
  横方向の仮想化は「グリッドと同じ計算を横に使う」だけだが、
  **`scrollIntoView` による中央寄せと roving tabindex が窓の外の要素を
  掴めなくなる**ので、送り先の枠が窓外にあるときの経路を別に書く必要がある
- 実測: 3,000 枚でプレビューを開いてからストリップが出るまでの体感の遅れは無く、
  `e2e/grid.spec.ts` のプレビュー 4 件はいずれも 0.6 秒以内に収まる

**Task 15 で計測した結果、入れないことに決めた。** ストリップを開いて 3,000 個の
`<button>` を DOM に置いたままグリッドをスクロールしても、`rAF` 間隔の p95 は
16.7ms・32ms 超のフレームは 0% で、閉じた状態と一切変わらない
（`e2e/scroll.spec.ts` の 3 番目の検査が毎回この条件を測り、
前提条件として枠が 3,000 個あることを assert している）。
**要素数そのものはこの規模では指標に出ない。** 入れると
`scrollIntoView` の中央寄せと roving tabindex が窓外の枠を掴めなくなる分の
経路が増えるだけなので、割に合わない。

### Task 14 — 位置表示は情報バーのものを残した（`.position` を足していない）

計画 Step 3 はフィルムストリップの上に `{currentIndex + 1} / {images.length}` の
`.position` を置くが、**現行の `ImagePreview` は情報バーの右端に同じものを既に
持っている**。両方出すと同じ数字が 2 つ並び、計画 Step 8 の
`viewer.getByText("1 / 3000")` も strict mode で落ちる。情報バー側を残し、
`tabular-nums` だけそちらへ移した。

**spec §4-1 の「現行は ← → で送れるが『いま何枚目か』が見えない」は正確ではない。**
枚数は現行も出ている。実際に見えていないのは**前後にどの写真があるか**で、
フィルムストリップが埋めるのはそちらである。

### Task 14 — 情報バーとストリップは 1 つの箱に積んだ

どちらも `position: absolute; bottom: 0` にすると重なる。`.bottom` で括り、
その `clientHeight` を測って `--preview-max-h`（写真の最大高）と
オーバーレイの `padding-bottom` に渡している。固定値で空けると、
ストリップの高さを変えたときに写真が箱の裏へ回る。
**`padding-bottom` の方を忘れると、写真は「画面全体の中央」に置かれて
下半分が箱に隠れる**（最大高だけでは足りない）。

### Task 14 — scrim の上の操作は「地の色を持つチップ」にした

`styles/` 以外に生の色を書けないので、`rgba(255,255,255,.7)` 系は全部落とした。
**scrim（黒）の上に置ける文字色のロールは無い** ── `on-surface` 系はライト時に
黒、`inverse-on-surface` はダーク時に黒で、どちらも片方のテーマで沈む。
閉じる・送り・選択・EXIF・読み込み中は
`background: inverse-surface` ＋ `color: inverse-on-surface` の**対**にした。
チップとして自己完結するので、両テーマで読める（両方のスクリーンショットで確認）。

選択矩形の「内側の薄い塗り」と「外側の暗転」は `::before` / `::after` に分け、
不透明度を疑似要素側に持たせた。要素自身に `opacity` を掛けると破線の枠まで
薄くなって見えなくなる。

### Task 14 — 要求種別の取り違えは e2e では検出できない

計画 Step 3 は「`discardable` にするとスクロールのたびに捨てられて永久に埋まらない」
と書くが、**`pinned` を `discardable` に変えても e2e は全部緑のままだった**（実測）。
理由は `thumbnailQueue.request` が `values`（`SvelteMap`）を読むこと ──
グリッドのサムネイルが 1 枚届くたびにストリップの `$effect` が再走し、
捨てられた要求が出し直される。**自己修復してしまうので画面からは差が出ない。**

`e2e/stub.ts` に `get_thumbnail` の遅延オプションを足して待ち行列を溜めるところまで
試したが（`setVisibleRange` が実際に 21 件全部を捨てるのは確認した）、
それでも最終状態は同じだったので、オプションごと戻した。
`e2e/grid.spec.ts` の当該検査は「枠が実データで埋まる」までの保証に改め、
**種別の規則は `requestQueue.test.ts`（純粋ロジック）に任せる**と明記した。
`pinned` のままにしてあるのは、捨てて出し直す往復が無駄だからで、
「そうしないと埋まらない」からではない。

### Task 14 — ズームのドラッグは e2e で確かめた

Step 10 の実機確認（唯一「移設したが触っていない」部分）は、
`page.mouse` で実際に矩形をドラッグして `img.preview-image` の
`style.transform` に `scale(` が乗ること、クリックで消えることを見た。
拡大前に transform が空であることを前提条件として先に assert してある
（最初から乗っていたら、この検査は何も検出しない）。

### Task 15 — 計測結果と、ベースラインの取り方

4 条件すべて `rAF` 間隔 p95 = **16.7ms**、32ms 超のフレーム **0%**。
60Hz の 1 フレームに張り付いており、落ちたフレームが 1 枚も無い
（値は spec §7-2 の表に転記した）。仮想スクロールの導入で悪化していないどころか、
3,000 枚でもフィルムストリップを開いても指標が動かない。

**ベースラインを測るときに `playwright.config.ts` の viewport も直した。**
worktree（7be9bbe）側は Task 13 で直した viewport のずれ（`devices` に
上書きされて 1280 で走る）をまだ持っており、**そのままだと新旧で画面幅が違う**。
実装は一切触らず、測定条件だけを 1440×800 に揃えた。

**p95 の比較は 0.1ms に丸めて行う。** 16.7ms は vsync の周期そのもので、
実測値は `16.700000000000728` のような形で返る。丸めずに
`<=` で比べると、実装差ではなく浮動小数の末尾で落ちる（実際に 1 度落ちた）。

**この検査はマシンの負荷に弱い。** 1 度、別プロセスで worktree を消しながら
走らせたときに 50 枚の比較だけが落ちた（単独で走らせると通る）。
spec §7-2 の「他のアプリを閉じた状態で測る」はこの検査にもそのまま効く。
3 回の中央値を取っているので単発のノイズは吸うが、計測中ずっと重い処理が
走っていると吸えない。

### Task 15 — LRU 上限は e2e の実測値では決められなかった

計画 Step 6 は「Step 5 の出力の `bytesPerThumbnail` から決める」と書いているが、
**その値（3.8KB）をそのまま使ってはいけない。** `e2e/stub.ts` のサムネイルは
2 色のグラデーションで、JPEG が最も得意とする絵柄である。実際の写真より
1 桁小さく焼けるので、そこから出した上限は実機で 1 桁足りない。

同じ経路（canvas → JPEG 品質 0.75 → base64）で絵柄だけ変えて測り直した:

| 絵柄 | 320px | 512px |
|---|---|---|
| スタブのグラデーション | 3.8KB | ─ |
| 帯域制限ノイズ（写真の細部に近い） | 6.6KB | 15.1KB |
| 一様ノイズ（病的な上限） | 59.6KB | 151KB |

1 枚 **20KB** を採った（帯域制限ノイズの 512px に余裕を持たせた値。実写は
これより細かい）。20KB × 3,000 枚 × 解像度 2 種類 = 120MB → **128MB**。
**低すぎる方が高すぎる方より悪い**（往復のたびに IPC と Rust のデコードを
やり直す）ので、迷ったら高い側へ倒してある。

`CACHE_BYTE_LIMIT` は **`thumbnailCache.ts`（純粋）へ移した。**
`thumbnailQueue.svelte.ts` は runes を持つので、e2e から import すると
Node 側で `$state` が解決できずにテストの収集ごと落ちる。上限は台帳の性質で
あって取得キューの性質ではないので、置き場としても純粋モジュール側が正しい。

### Task 15 — 上限そのものは e2e では踏めない

計画 Step 7 の「キャッシュはバイト上限を大きく超えない」は、スタブの
サムネイルが小さすぎて **128MB に到達できない**（3 解像度を往復して 26.8MB）。
到達しない以上「超えない」は自明に成立し、検査になっていない。

意味のある方の性質に振り替えた: **解像度を変えて往復すると台帳の件数が増える**
（`path:maxDimension` をキーにしている＝ `path` だけをキーにすると増えない）。
実測 3,000 件 → 7,390 件。上限の比較は残してあるが、これは番人であって
主眼ではない。追い出しの規則そのものは `thumbnailCache.test.ts` が見ている。

### Task 16 — 実機で改名を確かめた（ディスクの中身まで見た）

**Task 7 以降で唯一、実機のバックエンドに対して確かめられた項目がある。**
GUI ウィンドウは相変わらず操作できないが、改名の実体は
「`save_preset`（新名）→ `delete_preset`（旧名）」という 2 つの IPC の並びであり、
**結果はファイルとして観測できる**（Task 9 の `favorites.json` と同じ手）。

`src/main.ts` に一時プローブを足し、起動時に `probe-a` を保存 →
`probe-b` で保存 → `probe-a` を削除、という改名そのものの並びを走らせた。
`vite dev` を 5173 に上げてから debug バイナリを起動する（Task 9 の実施メモのとおり、
debug は `devUrl` を読む）。結果:

```
~/.config/picture-tool/presets/
└── probe-b.json   （中身の name も "probe-b"）
```

**`probe-a.json` は残っていない。** e2e はスタブ相手なので、
「複製ではない」は結局こちらが書いたモックの意味論を確かめているだけだった。
実ファイルが 1 つになることは、ここで初めて言えた。プローブはコミットしていない。
`~/.config/picture-tool/` はプローブ以前に存在しなかったので、確認後に丸ごと消してある。

**それでも言えないこと**: 画面を操作したときの一連の流れ（一覧のダブルクリック →
入力 → 保存ボタン）。押下から IPC までの配線は e2e が見ており、
IPC からファイルまでは上のプローブが見ているが、通しでは見ていない。

### Task 16 — トーストが右パネルの主ボタンを覆う（Task 18 の宿題）

**e2e が 2 件だけ 4〜5 秒かかることから見つけた。** Playwright が
`保存` ボタンのクリックを `<div role="region" aria-label="通知"> subtree intercepts
pointer events` で 500ms ごとに再試行していた。

`Toast.svelte` の `.toast-stack` は画面右下の `position: fixed` で、
`.toast` 個々は `pointer-events: auto` を持つ。パネル最下部の `.action`
（Task 10 で導入。spec §5-1 の「主ボタンはパネル最下部に固定」）はちょうど同じ場所にある。
**保存すると成功トーストが出て、消えるまで 4 秒間その下のボタンが押せない。**
エラートーストなら 8 秒。変換パネルの「N 枚を変換」も同じ位置なので、
これは本タスクが持ち込んだものではなく Task 6 と Task 10 の組み合わせで既に存在していた。

**本タスクでは直していない。** 直す場所は `Toast.svelte` の配置で、
それは spec §5-1 と §1 に関わる設計判断であり、フレームモードの範囲ではない。
**Task 18（段階 9 の総点検）で決めること。** 候補は「スタックを左下へ寄せる」
（左カラムの下端には主導線が無い）か「主ボタンの高さぶん `bottom` を上げる」。

e2e 側は待たせず明示的に閉じる（`e2e/frame.spec.ts` の `dismissToasts`）。
**待てば通るので放置もできるが、トーストの寿命を延ばした瞬間に落ちる**うえ、
本物の重なりを検出したいときに 4 秒の再試行と区別が付かなくなる。

### Task 16 — 記号だけが違う名前への改名は、プリセットが消える（core 側の問題）

`core/src/exif_frame/preset.rs` の `sanitize_filename` は、英数字と `-` `_` 以外を
すべて `_` に潰す。**この写像は単射ではない。**

```
"my preset" → my_preset.json
"my.preset" → my_preset.json   ← 同じファイル
```

改名は「新名で保存 → 旧名を削除」なので、両者が同じファイルに落ちる組み合わせでは
**いま書いたファイルを消す**ことになり、プリセットが丸ごと消える。
`from !== preset.name` の判定は名前で見ているので素通りする。

**日本語名は無事である**（実測で確認）。Rust の `char::is_alphanumeric` は
Unicode 準拠で、CJK 統合漢字もかなも Alphabetic なのでそのまま残る
（`夜景 → 夜景.json`）。潰れるのは空白・記号・`/`・`.` などだけ。

**本タスクでは直していない。** 根本原因はファイル名の写像であって GUI ではなく、
直す場所は `core/` である ── Global Constraints の「`core/` と `cli/` は一切触らない」に
正面から当たる。フロント側で同じ潰し方を書き写して衝突を弾く手も考えたが、
**Rust の実装詳細をフロントに複製することになり、片方だけ直る事故を作る**ので採らない。
「保存し直して自己修復する」形も、欠陥を隠して Task 18 と core の修正から見えなくする。

同じ衝突は**通常の保存でも既に起きている**（記号だけ違う 2 つのプリセットは
片方が片方を黙って上書きする）ので、これは改名が新しく作った欠陥ではなく、
改名によって「上書き」が「消滅」に悪化した既存の欠陥である。
本刷新の外の課題として記録する。

### Task 16 — 計画に無い配線を 2 つ足した

どちらも「押した結果が黙って食い違う」種類で、計画の Step 6 のままだと成立しない:

- **編集中のプリセットを削除したときに選び直す**（`App.handleDeletePreset`）。
  計画は `onDelete={presets.remove}` を直に渡すが、消したあとも `editingName` は
  消えた名前を指したままになる。その状態で保存を押すと **消したはずのプリセットが
  ディスクに書き戻る**（`isNew` も false なので画面上は普通の「保存」に見える）。
  `e2e/frame.spec.ts` の「削除したプリセットは、その後保存しても復活しない」がこれを見ている
  （再選択を消すと落ちることを実測で確認済み）
- **`onEditFrame` を `handleModeChange` に通した**（`App.svelte`）。Task 9 以来
  `mode = "frame"` の直代入で、計画 Step 6 が `handleModeChange` に足す
  フレームの初期化を通らない。変換パネルの「プリセットを編集...」から入ると
  `frame.draft` が null のままで、**右パネルが空になる**（`{#if draft}` の中なので）

### Task 16 — 改名の確定は二重に走らせない

`PresetList` の改名入力は Enter と blur の両方から確定する。計画のコードは
`commitRename` に番人が無く、**Escape で消した直後に blur が飛ぶと、
取り消したはずの名前で確定する**（ブラウザによって挙動が違う）。
Enter → 入力欄が消える → blur、の順でも二度呼ばれる。
`renaming` を先に `null` にしてから通知する形にして、両方を塞いだ。

ロケータも計画の `page.locator("input.rename")` から
`getByRole("textbox", { name: "プリセット名" })` に変えた。クラス名は実装の構造で、
`.rename` を別名にしただけでテストが落ちる。入力欄には `aria-label` を付けてある。

### Task 16 — e2e の SegmentedButton は `selectSegment` で押す

計画 Step 7 のテストは `page.getByRole("radio", { name: "白" }).click()` と
`{ name: "Pad" }` を直接クリックするが、**Task 10 の実施メモが「通らない」と
記録済み**である（`input` は隠してあり、当たり判定は `.text` が取る）。
`e2e/stub.ts` の `selectSegment` に差し替えた。`toBeChecked()` の側は
アサーションなのでそのまま `getByRole("radio")` で引ける。

`FramePanel` の `.action` には警告文・削除・保存の最大 3 つが積まれるので、
`ConvertPanel` の `.action` をそのまま写さず `gap` を持たせた
（Task 6 の「`Card` を積むところは親が間隔を持つ」と同じ話）。

### Task 17 — `App.svelte` は 180 行に収まらなかった（275 行）

計画 Step 4 の目安は 180 行以下、spec §3-5 は 150 行程度。**収まらなかった。**
「4 状態とパネルの差し替え」に当たらないものは 4 つ切り出してある:

- **変換の設定を `convertRun` へ移した。** 出力先・`ProcessingConfig`・
  Exif フレームの有無・元ファイル削除の確認は、使うのが「変換の実行」だけである。
  spec §3-5 が `convertRun` に与えた役目（実行・進捗・キャンセル・結果）より広いが、
  **選択（どの写真を変換するか）は App が持ったまま**で、`request()` の引数で渡す
- **全画面プレビューを `PhotoGrid` の内部状態にした。** 開く経路はグリッドだけで、
  `PhotoViewer` が要る props（一覧・選択・サムネイル）はグリッドが全部持っている。
  App から `previewImage` と 2 つのハンドラーと `onPreview` prop が消えた
- **フォント一覧を `FramePanel` が自分で読むようにした。** 使うのはこのパネルだけ。
  副次的に、起動後に `assets/fonts/` へ足したフォントもモードを開き直せば出る
- **`createFrameDraft` がプリセットストアを受け取る形にした。** 保存（改名込み）と削除は
  一覧と下書きの両方を触るので App が繋いでいたが、下書き側に寄せた。
  `select(name, presets.presets)` の第 2 引数を 3 箇所で渡す配線も消えた

残った 275 行の内訳は、4 状態＋ストア生成＋派生（約 60 行）、フォルダー・選択・
フォーカス・モードのハンドラー（約 50 行）、**3 モード × 3 カラムの差し替え
（約 85 行）**、オーバーレイ 4 つ（約 30 行）、import（26 行）、コメント（44 行）。
**spec §3-5 の 150 行は、`frameDraft` と `metadataDraft` と 3 モード分の
差し替えマークアップが無い前提の見積もりだった。**これ以上削るには
「パネルの差し替え」自体を別コンポーネントへ出すことになり、
それは spec が App に残すと決めたものである。

`createFrameDraft` の中で `save` / `remove` から `select` を呼ぶのに `this` は使えない。
`onSelect={frame.select}` のように**メソッドを剥がして渡す**ので `this` が失われる。
`select` はクロージャの関数にしてある。

### Task 17 — パネルの表示を見るロケータは右カラムに絞る

`page.getByText("photo-0000.jpg")` はグリッドのタイルとメタデータパネルの
両方に当たり、strict mode で落ちる。**落ちるのは正しい**（どちらを見ているのか
曖昧なテストになる）ので、`[data-region="right"]` で絞る形にした。
`data-region` は Task 18 Step 4 が Tab 順の検査のために要求している属性で、
ここで先に入れた。

### Task 17 — スクロール検査のベースラインが境界値に張り付いていた

`e2e/scroll.spec.ts` の 50 枚比較が **3 回中 2 回落ちる**状態だった。
本タスクの変更を stash して前のコミットで測っても同じ割合で落ちるので、
持ち込んだものではない。

原因は比較の分解能。60Hz の `rAF` 間隔は 16.667ms で、Chromium は
タイムスタンプを 0.1ms へ量子化するので実測は **16.7 と 16.8 が交互**に返る。
p95 がどちらの tick に落ちるかは同じ実装でも走らせるたびに変わるのに、
Task 15 のベースライン（16.7）は低い側の tick に張り付いていた。
`round1` は浮動小数の末尾を落とすだけで、この量子化は落とせない。

**vsync 1 tick 分の許容を入れた。** フレーム落ちは 33ms 台（tick 2 つ分）に
なるので、この許容では隠れない。`jankRatio` の比較もそのまま残してある。

### Task 18 — スクリーンショットが 150ms フェードの途中で撮れていた

Step 7 の目視用に撮った 6 枚のうち、**モードを切り替えて撮った 4 枚は
右パネルが真っ白**だった。パネル差し替えの 150ms フェード（spec §3-3）の
途中で `page.screenshot()` が走っていた。`aria-current` を待っても意味が無い
（モードは即時に立ち、遅れているのは opacity）。

`page.screenshot({ animations: "disabled" })` で有限のアニメーションを
完了状態へ送ってから撮る形にした。**これに気付かないまま Step 7 を
「目視した」と書くところだった。**

撮る前に写真を 1 枚選ぶようにもした。選ばないとメタデータもフレームも
「対象なし」の空表示になり、見たいものが写らない。

### Task 18 — フレームのプレビューが、要求する前から「生成できませんでした」と言う

Step 7 の目視で見つけた。`FramePreview` は生成を 300ms debounce しており、
`loading` を `setTimeout` の**中**で立てていた。待っている 300ms のあいだ
`!loading && !src` になるので、**まだ 1 度も要求していないのに失敗の文言が出る**。
フレームモードへ入るたびに 300ms 出ていた。`loading` を debounce の前に立てて直した。

**この検査は `toBeHidden()` では書けない。** 自動リトライが 300ms を待ち越して
しまい、窓の中に何が出ていたかを見ずに通る（欠陥を戻して実測し、通ることを確認した）。
`MutationObserver` で画面に出た文字列を全部記録し、その履歴に失敗の文言が
無いことを見る形にした。こちらは欠陥を戻すと落ちる。

### Task 18 — トーストは左下へ移した（Task 16 の宿題の結論）

候補は「左下へ寄せる」と「主ボタンの高さぶん `bottom` を上げる」だった。
**後者は採れない。** `.action` の高さはモードごとに違う（メタデータは
ボタン 2 段、フレームは警告文＋削除＋保存で最大 3 段）ので、固定のオフセットでは
追随しない。左下は 3 モードとも押すものが無い（フォルダーツリーの余白、
プリセット一覧の説明文）。

左端は rail の幅ぶん内側から始める。rail 幅 80px は
**NavigationRail の CSS・AppShell のドラッグ計算・Toast の左端**の 3 箇所が
同じ値を見る必要があるので、`--rail-width` として tokens.css に置いた
（3 つ目の写しを増やすより、既にあった 2 つの写しを 1 つにする方が良い）。
AppShell の JS 定数だけは座標計算に数値が要るので残し、コメントで対にしてある。

重なりを検査する e2e を足した（`e2e/a11y.spec.ts`）。トーストと主ボタンの
矩形が交差しないこと＋「消えるのを待たずにもう一度押せる」こと。右下へ戻すと落ちる。
`e2e/frame.spec.ts` の `dismissToasts`（重なりを避けるための回避策）は消した。
**回避策を残すと、重なりが戻ったときに検出できない。**

### Task 18 — Tab 順の検査は起点を文書の先頭へ戻さないと成立しない

フォルダーを開くのにツリーをクリックするので、フォーカスは左カラムの中にある。
そこから Tab を押しても rail は通らない（一巡するまで戻って来ない）。
**`blur()` では戻らない** ── sequential focus navigation starting point は
直前の要素に残る。`document.body` に `tabindex="-1"` を付けて `focus()` する。

Tab の回数は固定にせず「右パネルへ着くまで（上限 80）」にした。左カラムの
ツリーは項目数だけ Tab を消費するので、固定回数だと届かないまま打ち切られ、
順序を検査したことにならない。計画のテストは 12 回押して
「2 種類以上の領域を通った」しか見ておらず、**順序そのものを検査していなかった**。

計画に無い検査をもう 1 つ足した: **リサイザーのキーボード操作**（Step 9 の
手動項目のうち、e2e に無かった唯一のもの）。Tab だけで到達できることと、
← → が 16px ずつ動かすことを見る。

### Task 18 — 生の色は 0 件。`font-size` の生値は 9 箇所残っている

Step 3 の grep はどれも 0 件で通った（旧 `app.css` 変数、`#rrggbb` / `rgba()`、
`gap` / `margin` / `padding` の生 px、`border-radius` の生値、`transition` の生時間）。

`font-size` の生値だけ 9 箇所ある。**すべて装飾記号の字面の大きさ**で、
`Button` / `FolderTree` の `.icon`、`Rating` の `.star`、`Dialog` の閉じる ✕、
`PhotoViewer` の閉じる・前後送り、`PhotoGrid` の 📷、rail の pill の中身。
Global Constraints の「部品固有の内部寸法」に当たるので残した。
**本文のテキストに生の `font-size` を書いているところは無い。**

### Task 18 — 実機 `rAF` と Chromium の差

**未実施。この項目だけが残っている。**

Step 8 は Tauri 実機（`make dev`）の webview で devtools コンソールに
測定関数を貼って走らせることを求めているが、WebKitGTK の webview は
外から自動で操作できず、意味のある数字を出すには**実写 3,000 枚のフォルダー**も要る
（スタブのグラデーションで測っても、Task 15 で分かったとおり実機の代理にならない）。
Chromium 側の値は spec §7-2 の表にある（4 条件すべて p95 16.7ms / 32ms 超 0%）。

**やるときの手順**（計画 Step 8 から 1 箇所直してある）:

1. `make dev` で起動し、実写が 3,000 枚あるフォルダーを開く
2. webview の devtools を開き、`e2e/scrollPerf.ts` の `page.evaluate` に
   渡している関数の中身をコンソールへ貼る
3. **セレクタは `.scroller`。** 計画は `'[role="listbox"][aria-label="写真"]'` と
   書いているが、Task 14 で**スクロールする箱と `listbox` を別の要素に分けた**ので、
   listbox を渡しても `scrollHeight - clientHeight` が 0 になり測定にならない
4. 得た p95 と 32ms 超の割合をここへ追記する。桁が違ったら
   spec §7-2 の「限界」の記述を更新する（測定を捨てるのではなく、
   何が分かって何が分からないかを書き残す）
...
- **スクロールする箱と `listbox` を別の要素に分けた**（Task 14）。spec §4-1 の
  「スペーサー要素を listbox の直接の子にしてはならない」は満たしているが、
  余白をスクローラー自身の padding で作ると padding が要素の箱を膨らませて
  スクロールしなくなる。listbox はスクロールする箱ではない
- **全画面プレビューの位置表示を増やしていない**（Task 14）。spec §4-1 は
  「現行は『いま何枚目か』が見えない」と書くが、現行も情報バーに出ている。
  実際に見えていないのは前後にどの写真があるかで、そちらはストリップが埋める
...
- **`App.svelte` は 150 行にならず 275 行**（Task 17）。spec §3-5 の見積もりに
  `frameDraft` / `metadataDraft` と 3 モード分の差し替えマークアップが入っていない。
  切り出せるものは 4 つ切り出したうえでの数字で、残りは spec が App に残すと
  決めたもの（詳細は Task 17 の節）
- **`convertRun` が変換の設定も持つ**（Task 17）。spec §3-5 は
  「変換実行・進捗・キャンセル・結果」と書いているが、出力先・`ProcessingConfig`・
  Exif フレームの有無もここに置いた。選択は App が持ったまま
- **全画面プレビューは `PhotoGrid` が持つ**（Task 17）。spec §4-1 は
  プレビューをグリッドと並ぶものとして書いているが、開く経路がグリッドしか無く、
  必要な props も全部グリッドにある
- **`--rail-width` をトークンに足した**（Task 18）。spec §1 のトークン層は
  色・形・余白・タイポ・状態・モーションで、構造的な寸法を含まない。
  rail 幅は 3 箇所（rail の CSS・シェルのドラッグ計算・トーストの左端）が
  同じ値を見る必要があるので例外にした
- **トーストは右下ではなく左下**（Task 18）。spec は位置を定めていないが、
  右下は 3 モードとも主ボタンの位置で、重ねると押せなくなる（Task 16 の宿題の結論）
- **`FramePreview` の debounce 中は「読み込み中」**（Task 18）。spec は
  プレビューの状態表示を定めていないが、要求前に「生成できませんでした」と
  出すのは事実に反する
