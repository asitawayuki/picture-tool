...
- **選択と展開の関係:** クリック時は常に`onSelectFolder`を呼び出してフォルダーを選択状態にした上で展開/折りたたみをトグルする。折りたたんでも選択状態（`selectedPath`）は維持される。これはファイルマネージャの標準的な動作に合わせた設計
...
- `SelectionList`にも`onRequestThumbnail`コールバックを渡す。選択リストに追加された画像のサムネイルがキャッシュに存在しない場合（グリッドの可視領域外から選択された場合）、`SelectionList`が表示時にロードを要求する
...
| `lib/SelectionList.svelte` | 共有キャッシュ参照によるサムネイル表示、`onRequestThumbnail`コールバックでキャッシュ未ヒット時にロード要求、ダブルクリックプレビュー |
...
| `state.rs` | サムネイルLRUキャッシュ追加（`std::sync::Mutex<lru::LruCache<String, String>>`、上限500エントリ。`get_thumbnail`は`async fn`だがキャッシュアクセスは`await`をまたがないため`std::sync::Mutex`で十分） |
...
| `gui/capabilities/default.json` | store関連permission追加（`store:allow-set`, `store:allow-get`, `store:allow-save`, `store:allow-load`） |
