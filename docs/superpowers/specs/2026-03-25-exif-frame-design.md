...
> **Superseded by v2** — 現行仕様は
> [`2026-03-29-exif-frame-v2-design.md`](./2026-03-29-exif-frame-v2-design.md)。
> 本書のうち、レイアウト選択（BottomBar/SideBar/FullBorder）、フレーム色・アスペクト比の
> 個別指定、型番の表示名変換（`ILCE-7M4` → `α7IV`）は **v2 で廃止**され、実装にも存在しない。
> 履歴として残しているだけなので、実装の根拠には使わないこと。
