...
- セパレーター: **1行目のみ**半角パイプ `" | "`。2行目（撮影パラメータ）は半角スペース2個で区切る
  （`f/2.8 | 1/250s` のように短い値をパイプで割ると視覚的に密になりすぎるため）
- テキスト色: 背景の輝度（`0.299R + 0.587G + 0.114B`）で判定。閾値128未満（暗い）→ `#ffffff`（1行目）/ `#aaaaaa`（2行目）、128以上（明るい）→ `#333333` / `#888888`
  - **輝度計算は任意RGBに対応しているが、入力側の `BackgroundColor` は現状 White/Black の2値 enum**。
    カスタムRGBを受け付ける口はまだ無い（描画側は変更不要で入力の型を広げれば足りる）

### 縦構図：右余白に2段・90度回転

> **設計変更（実装時に判断を逆転、コミット `5273d30`）** — 当初は「1行に凝縮して回転」と
> していたが、横構図と同じ2段レイアウトを一時バッファに描画してから90度回転する方式に変更した。
> 理由は、1行凝縮だとテキストサイズ・ロゴ配置・レンズロゴの計算が横構図と別系統になり、
> **同じ内容なのに構図によって見た目と実装が二重化する**ため。2段のまま回転すれば計算は1つで済み、
> 表示内容も構図によらず一致する。以下の図は当初案の記録として残す。
...
- 実装は横構図と同じ2段（1段目: [Sonyロゴ] | カメラ型番 | レンズ型番 [GMロゴ] /
  2段目: 焦点距離 F値 SS ISO）を一時バッファに描画し、90度回転して合成する
- 余白の縦方向全体を使って配置
- 写真はExifバー除外領域ではなく**キャンバス全体に対して**中央配置する
  （左右の余白が視覚的に均等に見えるため。コミット `22f6192`）

**テキストオーバーフロー時の対応:**
1. まず自動フォントサイズ縮小
2. それでも収まらない場合は末尾を `...` で切り詰める
3. 余白自体が足りない場合は写真を縮小して帯を確保する（縮小率20%が上限。
   超える場合はExifフレームをスキップして通常のpad出力にフォールバック）

> **未実装** — 「優先度の低い項目から省略（date_taken → custom_text → shutter_speed →
> iso → focal_length）」は入れていない。切り詰めで実用上は足りており、
> 省略順の指定は設定項目を増やす割に効果が薄いと判断した。必要になったら
> `text.rs` の切り詰め処理の手前に挟むこと。
...
- **レンズブランドロゴ（GM）**: **Exifエリアの右端**に配置

> **設計変更** — 当初は「レンズ型番の直前にインライン」としていたが、テキストの途中に
> 画像を挟むとレイアウト計算がテキスト描画と絡み合い、切り詰め・回転との組み合わせで
> 破綻しやすかった。右端に固定すれば、テキスト側は「ロゴの手前まで」という幅の制約を
> 受け取るだけで済む。テキストとロゴが重ならないことは `render_exif_frame` の
> 統合テストで固定している（S4-H6）。
