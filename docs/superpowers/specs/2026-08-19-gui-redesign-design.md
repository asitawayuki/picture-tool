...
測定機: `Linux biwak 7.1.8-200.fc44.x86_64`（開発機の論理解像度 3072×1920）。
Playwright（Chromium）/ `vite dev` / `e2e/stub.ts` のスタブサムネイル /
viewport 1440×800。最上部から最下部まで 6 秒の一定速度スクロール、
各条件 3 回の中央値（`e2e/scroll.spec.ts` / ベースラインは
`e2e/scroll-baseline.json`）。

| 条件 | `rAF` 間隔 p95 | 32ms 超のフレーム割合 |
|---|---|---|
| ベースライン（現行実装 / 50 枚） | `16.7 ms` | `0 %` |
| 新実装 / 50 枚 | `16.7 ms` | `0 %` |
| 新実装 / 3,000 枚 | `16.7 ms` | `0 %` |
| 新実装 / 3,000 枚・フィルムストリップを開いた状態 | `16.7 ms` | `0 %` |

**4 条件すべてが 60Hz の 1 フレーム（16.7ms）に張り付き、32ms を超えたフレームは
1 枚も無い。** 合格条件（2 つの統計量がどちらもベースライン以下）を満たす。
なお 16.7ms は vsync の周期そのもので、浮動小数の末尾まで比べると実装差ではなく
測定ノイズで大小が決まる。比較は 0.1ms に丸めて行う。

**丸めても、この測り方の分解能は vsync 1 tick である。** 60Hz の 1 フレームは
16.667ms で、Chromium はタイムスタンプを 0.1ms へ量子化するため、実測の間隔は
**16.7 と 16.8 を交互に返す**。p95 がどちらの tick に落ちるかは同じ実装でも
走らせるたびに変わる（段階 9 で、変更前のコミットが 3 回中 2 回 16.8 になることを
実測した）。したがって**ベースラインとの比較は 1 tick の許容を持たせる**。
この測り方で意味を持つ悪化はフレーム落ち（33ms 台 ＝ tick 2 つ分、
および 32ms 超の割合が 0 でなくなること）であって、tick 1 つ分の差ではない。

4 行目は、全画面プレビューのフィルムストリップを**仮想化しない**判断
（写真の枚数だけ `<button>` を置く）の裏づけとして取った。3,000 要素を DOM に
置いたままグリッドをスクロールしても指標が動かないので、横方向の仮想化は入れていない。

サムネイルキャッシュの実測: 3,000 枚を 1 往復した時点で **3,000 件 / 11.5MB**、
1 枚あたり **約 3.8KB**。サイズを 3 種類に振って往復すると **7,390 件 / 26.8MB**。

**ただしこの 3.8KB を LRU 上限の根拠にしてはならない。** スタブの絵は単調な
グラデーションで、実際の写真より 1 桁小さく焼ける。同じ経路で絵柄だけ変えて測ると、
写真に近い帯域制限ノイズは 320px で 6.6KB・512px で 15.1KB、一様ノイズ（病的な上限）は
320px で 59.6KB になる。上限の根拠には後者側の値を使う（8 節）。
...
| サムネイルキャッシュの LRU 上限 | **→ 段階 6 で確定: 128MB**（`thumbnailQueue.svelte.ts` が `thumbnailCache.ts` の `CACHE_BYTE_LIMIT` を使う）。**上限 = 1 枚あたりのバイト数 × 保持したい枚数 × 解像度の種類**。保持したい枚数は「3,000 枚のフォルダーを 1 往復しても戻ったときに再取得が起きない程度」＝ 1 解像度あたり 3,000 枚、解像度はサイズスライダーで 2〜3 種類できるので 2 倍。1 枚あたりは **20KB**（7-2 の絵柄別の実測で、写真に近い絵の 512px が 15.1KB。実写はそれより細かいので余裕を見た値）。20KB × 3,000 × 2 = 120MB → 128MB。**上限が高すぎるのはメモリを食うだけだが、低すぎると往復のたびに IPC と Rust 側のデコードをやり直すことになる**ため、迷ったら高い側へ倒してある |
| rail 幅 80px | M3 の標準値を採ったが、日本語ラベル（「メタデータ」等）が収まるかは実装時に確認。収まらなければラベルを短くする（「情報」等）か幅を広げる。**→ 段階 4 で確定: 「変換」「情報」「フレーム」の 3 語なら 80px に収まる**（`e2e/shell.spec.ts` が rail 幅 80px とラベル幅を毎回検査する）。「メタデータ」は使わず「情報」を採った |
| `localStorage` の可用性 | Tauri の webview では origin が安定するため永続する想定。消えた場合はカラム幅が既定に戻るだけで実害は無い。ただし **WebKitGTK（Linux）は永続が不安定な例が知られており、開発機が Linux だと「毎回既定に戻る」体験になり得る**。段階 4 で実機を再起動して確認し、保たないようなら永続自体を落とす（可変幅は維持する）。**→ 段階 4 で確認済み: WebKitGTK でも永続した**（debug ビルドの `http://localhost:5173` と release ビルドのカスタムプロトコル、両 origin で確認）。永続は落としていない |
