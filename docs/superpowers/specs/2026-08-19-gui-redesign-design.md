...
| ウィンドウ幅 | グリッドに残る幅 | N=200 での列数 |
|---|---|---|
| 現行 1200（左 220 + 右 240） | 740 → 内側 716 | 4 列（現行の既定） |
| 新 1200（rail 80 + 240 + 右 320） | 560 → 内側 536 | **2 列** |
| 新 900（`minWidth`） | 260 → 内側 236 | **1 列** |

（グリッドの `padding: 12px` × 2、`gap: 8px` を引いた実値。
列数は `floor((内側 + gap) / (N + gap))`）

打ち消しとして 2 つ入れる:

1. **`gui/tauri.conf.json` のウィンドウ寸法を上げる** — 既定 `1200×800` → **`1440×900`**、
   `minWidth` `900` → **`1100`**。設定値の変更のみで、コマンドも権限も増えない
2. **右パネルを折りたためるようにする** — パネル頭の `IconButton` で幅 0 に畳む。
   写真を流し見する間はグリッドが全幅を使える。
   カラム幅はもともとドラッグ可変・永続化するので、折りたたみはその延長で実装できる
...
グリッドは可視範囲が変わるたびにキューへ `[開始 index, 終了 index]` を通知し、
キューはその範囲外の**未処理**要求だけを捨てる（処理中のものは捨てない）。
...
| 3 | **既存の周辺コンポーネント 5 個をトークンへ移行**（下記） | ライト／ダーク両方で 5 個が正しく描画される |
